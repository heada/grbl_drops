...
void st_wake_up()
{
  ...
  if (bit_istrue(settings.flags,BITFLAG_INVERT_ST_ENABLE)) { val |= (1<<STEPPERS_DISABLE_BIT); }
  else { val &= ~(1<<STEPPERS_DISABLE_BIT); }
...
void st_go_idle()
{
  ...
  uint8_t val = 0;
  if (pin_state) { val |= (1<<STEPPERS_DISABLE_BIT); }
  else { val &= ~(1<<STEPPERS_DISABLE_BIT); }
  rampsWriteDisable(val);
}
