...
uint8_t gc_execute_line(char *line)
{
  ...
  while (line[char_counter] != 0) { // Loop until no more g-code words in line.
    ...
    switch(letter) {
      ...
      default:
        ...
        switch(letter){
           //case 'A': word_bit = WORD_A; gc_block.modal.printer// Not supported
           case 'B': word_bit = WORD_B; gc_block.values.b = int_value; break; // Command B added for HP 45 printhead 
  ...
  */
  ...
  //[22. Send Print Data to Print Controller]:
  // The next lines send the Print Data in 8 bit blocks via TWI to the Drops controller
  if(bit_istrue(value_words,bit(WORD_B)))  
  { 
static uint8_t test;
	gc_block.values.p_buffer[buffer_num++]=gc_block.values.b;

	//gc_block.values.p_buffer[0]=gc_block.values.b;
	//for(int i = 0;i<sizeof(gc_block.values.p_buffer)/sizeof(gc_block.values.p_buffer[0]);i++)
	//{
		//gc_block.values.p_buffer[i]=gc_block.values.b+ii;
	//	gc_block.values.p_buffer[i]=line[i+1];
	//}
    twi_setAddress(1);
	//if(gc_block.values.buffer_num==22){
	if(buffer_num==sizeof(gc_block.values.p_buffer)/sizeof(gc_block.values.p_buffer[0])-1){
	switch (twi_writeTo(8,gc_block.values.p_buffer,sizeof(gc_block.values.p_buffer)/sizeof(gc_block.values.p_buffer[0]),true, true))
	{
		case 0: printString("success"); break;
		case 1: printString("1 .. length to long for buffer"); break;
		case 2: printString("2 .. address send, NACK received"); break;
		case 3: printString("3 .. data send, NACK received"); break;
		case 4: printString("4 .. other twi error (lost bus arbitration, bus error, ..)"); break;
	}
	buffer_num=0;
	}
	
	//}
 // print_uint8_base2_ndigit(gc_block.values.b,8);
   //print_uint8_base2_ndigit(sizeof(gc_block.values.p_buffer)/sizeof(gc_block.values.p_buffer[0]),8);
